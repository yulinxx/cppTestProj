/**
 * @file 3d_convex_hull.cpp
 * @brief CGAL 3D 凸包示例
 *
 * 什么是 CGAL 3D 凸包？
 * ====================
 * 3D 凸包是包含给定点集的最小凸多面体。它是2D凸包在三维空间的推广，
 * 由三角形面组成，包围所有给定的三维点。
 *
 * 3D 凸包的数学定义：
 * - 对于点集 S ⊂ R³，凸包 CH(S) 是包含 S 的最小凸集
 * - 凸包是一个凸多面体，由顶点、边和面组成
 * - 凸包的顶点是点集的极点（extreme points）
 *
 * 3D 凸包的重要性质：
 * 1. 最小性：凸包是包含所有点的最小凸多面体
 * 2. 凸性：任意两点的连线在凸包内
 * 3. 极点定理：凸包的顶点是原点集的子集
 * 4. 面：凸包的面是三角形或凸多边形
 *
 ...
 * 1. 维度增加：从多边形到多面体
 * 2. 面类型：2D是线段，3D是三角形
 * 3. 复杂度：O(n log n) vs O(n^2)
 * 4. 算法：QuickHull、增量算法、分治算法
 *
 * CGAL 3D 凸包算法：
 * 1. QuickHull 算法：平均 O(n log n)，最坏 O(n²)
 * 2. 增量算法：O(n²)，适合动态插入
 * 3. 分治算法：O(n log n)
 * 4. CGAL 使用精确算术处理退化情况
 *
