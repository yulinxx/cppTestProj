 ...
 * 什么是 CGAL 网格处理？
 * ====================
 * 网格处理（Mesh Processing）是对三角网格进行各种操作的技术。
 * 网格是由顶点、边和面组成的几何数据结构，广泛应用于计算机图形学
 * 和工程分析中。
 *
 * 网格的数学表示：
 * - 顶点：网格中的点 V = {v1, v2, ..., vn}
 * - 边：连接两个顶点的线段 E = {e1, e2, ..., em}
 * - 面：由边围成的多边形 F = {f1, f2, ..., fk}
 * - 拓扑关系：顶点-边-面的连接关系
 *
 * 网格的基本类型：
 * 1. 三角形网格：所有面都是三角形
 ...
 *
 * CGAL 提供的网格处理功能：
 * 1. 网格简化（Decimation）：减少三角形数量
 * 2. 网格细分（Subdivision）：增加三角形数量
 * 3. 网格平滑（Smoothing）：改善三角形质量
 * 4. 网格修复（Repair）：修复错误网格
 * 5. 曲面重建（Surface Reconstruction）：从点云重建网格
 * 6. 网格参数化（Parameterization）：UV映射
 * 7. 网格变形（Deformation）：形状修改
 *
 * 网格处理的应用场景：
 * 1. 3D打印：模型修复、网格简化
 * 2. 计算机图形学：实时渲染、动画
