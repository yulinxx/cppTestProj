 ...
 * 什么是 CGAL 多边形偏移？
 * ====================
 * 多边形偏移（Offset / Buffer）是创建多边形等距线的技术。
 * 给定一个多边形和一个距离，偏移操作生成一个新的多边形，
 * 其边界与原多边形的距离等于给定距离。
 *
 * 多边形偏移的数学定义：
 * - 对于多边形 P 和距离 d，偏移多边形 P⊕d 定义为：
 *   P⊕d = {x | d(x, P) ≤ d}
 *   其中 d(x, P) 是点 x 到多边形 P 的最短距离
 *
 * 偏移操作的类型：
 * 1. 外部偏移（Expansion）：向外扩展
 * 2. 内部偏移（Contraction）：向内收缩
 * 3. 单边偏移：只偏移一条边
 *
 * 偏移结果的几何变化：
 * 1. 直线边：平行向外移动
