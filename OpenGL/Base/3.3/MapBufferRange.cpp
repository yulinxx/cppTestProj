...
特性：                       glBufferSubData  OpenGL 1.1                  	glMapBufferRange  OpenGL 3.0

操作方式： 	        复制式：将 CPU 内存中的数据拷贝到 GPU 缓冲区     	    映射式：将 GPU 缓冲区内存直接映射到 CPU 地址空间，可直接读写

是否需要中间内存：         	是（提供一个源指针）	                        否（返回的指针就是缓冲区的一部分）
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(1280, 720, "glMapBufferRange Example", nullptr, nullptr);
    if (!window)
    {
