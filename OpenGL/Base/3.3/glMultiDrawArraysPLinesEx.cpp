...
glMultiDrawArrays 是 OpenGL 中用于高效绘制多个图元（primitives）的函数。
...
int main()
{
    ...
    std::vector<GLsizei> originalVertexCounts; // 保存原始的顶点数量，用于恢复显示
    ...
    for (int i = 0; i < NUM_LINES; ++i)
    {
        ...
        originalVertexCounts.push_back(numVertices); // 保存原始顶点数量
    ...
    // 所有线段默认可见（vertexCounts已设置为正确的顶点数量）
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (currentTime - lastUpdateTime >= UPDATE_INTERVAL)
        {
            ...
            // 随机选择部分线段显示/隐藏
            std::uniform_real_distribution<> visibilityDist(0.0f, 1.0f);
            int visibleCount = 0;
            for (int i = 0; i < NUM_LINES; ++i)
            {
                // 50%的概率切换线段可见性
                if (visibilityDist(gen) < 0.5f)
                {
                    // 如果当前是隐藏的（vertexCounts[i] == 0），则恢复显示
                    // 否则，隐藏该线段（设置vertexCounts[i] = 0）
                    if (vertexCounts[i] == 0)
                    {
                        vertexCounts[i] = originalVertexCounts[i]; // 恢复原始顶点数量
                    }
                    else
                    {
                        vertexCounts[i] = 0; // 隐藏线段
                    }
                }
            ...
            std::cout << "Updated visibility: " << visibleCount << " lines visible" << std::endl;
        }
        ...
        // 使用vertexCounts数组直接控制绘制，当vertexCounts[i]为0时不会绘制该线段
