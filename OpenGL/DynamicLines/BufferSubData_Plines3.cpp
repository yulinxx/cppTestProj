    ...
    size_t& eboUsedCount)
{
    ...
    freeList.clear();
    if (nextV < MaxVertices)
        freeList.push_back({ nextV, MaxVertices - nextV });
...
int main()
{
    ...
    for (int i = 0; i < 500; i++)
    {
        ...
        size_t vOffset;
        if (!allocateFreeBlock(freeList, pts, vOffset))
            break;
        ...
        m_vPlDatas.push_back(std::move(P));
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int u = 0; u < updates; ++u)
        {
            if (m_vPlDatas.empty())
                break;
            int id = rand() % m_vPlDatas.size();
            Polyline& P = m_vPlDatas[id];
        ...
        if (glfwGetTime() - opTimer > 1.0)
        {
            ...
            else
            {
                ...
                if (!m_vPlDatas.empty())
                {
                    ...
                    m_vPlDatas.erase(m_vPlDatas.begin() + id);
                }
            }
        }
        ...
        {
            // recompact
            defragmentBuffers(VBO, EBO, m_vPlDatas, freeList, MaxVertices, MaxIndices, eboUsedCount);
            lastDefrag = glfwGetTime();
            std::cout << "[Defrag] done. polylines=" << m_vPlDatas.size() << " eboUsed=" << eboUsedCount << " freeBlocks=" << freeList.size() << "\n";
        }
        ...
        if (now - fpsTimer >= 0.5)
        {
            ...
            frameCount = 0;
            char title[256];
            sprintf(title, "Polylines: %zu  FPS: %.1f  FreeBlocks: %zu", m_vPlDatas.size(), fps, freeList.size());
