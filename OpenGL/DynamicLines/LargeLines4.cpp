...
// ----------------------------- (内存分配器: 首次适应算法 + 合并) -----------------------------
// 从空闲块列表中分配一个大小为need的内存块
// 参数:
//   vFreeBlock: 空闲块列表
//   need: 需要的顶点数量
//   outOffset: 输出参数，返回分配到的起始偏移量
// 返回值: 成功返回true，失败返回false
bool allocateFreeBlock(std::vector<FreeBlock>& vFreeBlock, size_t need, size_t& outOffset)
{
    for (auto it = vFreeBlock.begin(); it != vFreeBlock.end(); ++it)
    {
        if (it->length >= need)  // 找到第一个足够大的空闲块
        {
            outOffset = it->offset;  // 记录分配位置

            if (it->length == need)  // 如果恰好完全使用
            {
                vFreeBlock.erase(it);  // 从空闲列表中移除该块
            }
            else  // 如果分配后还有剩余空间
            {
                it->offset += need;    // 空闲块起始位置向后移动
                it->length -= need;    // 空闲块长度减少
            }
            return true;
        }
    }
    ...
    size_t& eboUsedCount)
{
    ...
    // 先计算所有多段线的新位置，不修改原数据
    std::vector<std::pair<size_t, size_t>> newOffsets;  // 存储<新VBO偏移, 新EBO偏移>
    for (auto& p : polylines)
    {
        newOffsets.push_back({ nNextV, nNextI });  // 记录新位置
    ...
    {
        auto& p = polylines[i];
        auto [newVboOffset, newIndexOffset] = newOffsets[i];  // 获取新位置
    ...
    std::cout << "[Defragmentation] Completed. Polyline count=" << polylines.size()
