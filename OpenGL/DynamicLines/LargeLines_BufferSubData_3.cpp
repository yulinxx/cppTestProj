...
int main()
{
    ...
    for (int i = 0; i < initialLines; i++)
    {
        ...
        line.r1 = line.r2 = randomColorComponent();
        line.g1 = line.g2 = randomColorComponent();
        line.b1 = line.b2 = randomColorComponent();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 0; i < nUpdateLines; i++)
        {
            ...
            lines[nIdx].r1 = lines[nIdx].r2 = randomColorComponent();
            lines[nIdx].g1 = lines[nIdx].g2 = randomColorComponent();
            lines[nIdx].b1 = lines[nIdx].b2 = randomColorComponent();
        }
        ...
        // 上传当前所有线的数据（顶点 + 索引）
        //------------------------------------------------------
        // 每个顶点现在包含5个float(x,y,r,g,b)
        std::vector<float> vVertexData(lines.size() * 10);
        ...
        for (size_t i = 0; i < lines.size(); i++)
        {
            ...
            vIndexData[i * 2 + 0] = static_cast<unsigned int>(i * 2 + 0);
            vIndexData[i * 2 + 1] = static_cast<unsigned int>(i * 2 + 1);
        }
        ...
        if (currentTime - dFpsTimer >= 0.5)
        {
            ...
            char title[128];
            sprintf_s(title, 128, "Dynamic Lines + FPS | FPS: %.2f | Lines: %zu", fps, lines.size());
