...
int main()
{
    ...
    while (!glfwWindowShouldClose(win))
    {
        ...
        // 更新随机多段线
        for (int i = 0; i < 200 && !objPlineSlotMap.m_vPlDatas.empty(); ++i)
        {
            ...
            std::memcpy(mappedPtrs[nCurBuffer] + pl.nPtOffset * VERTEX_STRIDE, pl.verts.data(),
        ...
        if (glfwGetTime() - lastOp > 1.0)
        {
            ...
            else if (!objPlineSlotMap.m_vPlDatas.empty())
            {
                ...
                deallocate(arrFreeBlockMap[nCurBuffer], pl.nPtOffset, pl.nPtSz);
        ...
        if (glfwGetTime() - fpsTime >= 0.5)
        {
            ...
            snprintf(title, sizeof(title), "Polylines: %zu | FPS: %.0f | FreeBlocks: %zu | Buffer: %d",
                objPlineSlotMap.size(), fps, arrFreeBlockMap[nCurBuffer].size(), nCurBuffer);
