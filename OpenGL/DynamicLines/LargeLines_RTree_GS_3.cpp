/*
一个高性能的2D图元渲染程序，主要功能和特点如下：

核心功能：

- 使用OpenGL渲染大量2D图元（线条、圆、多段线），支持百万级图元的高效显示
- 实现了基于Boost.Geometry R树的空间索引，用于加速视口可见性计算
- 采用几何着色器（Geometry Shader）技术实现线宽、圆帽等高级线条渲染效果
- 支持交互式操作，包括鼠标平移和缩放功能
技术亮点：

1. 空间索引优化 ：使用Boost.Geometry的RTree进行空间分区，快速筛选可见图元
2. GPU加速渲染 ：利用几何着色器将基本线段扩展为带有圆帽的粗线条
3. 动态数据传输 ：采用orphan buffer策略优化GPU数据上传性能
4. 视口裁剪 ：只渲染视口范围内可见的图元，提高渲染效率
主要组件：

- 图元系统 ：包含Line、Circle、PolylinePrim等基础图元类，每种图元都有自己的光栅化实现
- 渲染器 ：管理着色器、缓冲区和绘制过程
- 相机系统 ：实现2D正交投影和交互式相机控制
应用场景： 此类程序适用于需要处理和显示大量几何图元的应用，如CAD系统、数据可视化工具、大规模图形编辑器等高性能图形界面。
...
void main(){
    ...
    // 主体四边形
    gl_Position = vec4((A - pixelNdir*pixelThick)/uViewportSize, 0.0, 1.0);
     fColor=col;
    fUV=vec2(0,0);
    EmitVertex();
    ...
    gl_Position = vec4((B - pixelNdir*pixelThick)/uViewportSize, 0.0, 1.0);
    fColor=col;
    fUV=vec2(1,0);
    EmitVertex();
    ...
    gl_Position = vec4((C - pixelNdir*pixelThick)/uViewportSize, 0.0, 1.0);
    fColor=col;
    fUV=vec2(1,0);
    EmitVertex();
    ...
    gl_Position = vec4((D - pixelNdir*pixelThick)/uViewportSize, 0.0, 1.0);
    fColor=col;
    fUV=vec2(0,0);
    EmitVertex();
    ...
    for(int i=0; i<=12; ++i){
        ...
        gl_Position = vec4((p0 + off)/uViewportSize, 0.0, 1.0);
        fColor=col; EmitVertex();
    }
    EndPrimitive();
    ...
    for(int i=0; i<=12; ++i){
        ...
        vec2 off = vec2(cos(ang), sin(ang)) * pixelThick;
        gl_Position = vec4((p1 + off)/uViewportSize, 0.0, 1.0);
        fColor=col;
         EmitVertex();
    }
    EndPrimitive();
...
public:
    ...
    ~Renderer()
    {
        glDeleteVertexArrays(1, &lineBatch.VAO);
        glDeleteBuffers(1, &lineBatch.VBO);
    ...
    void createShader()
    {
        ...
        GLuint g = compile(gs, GL_GEOMETRY_SHADER);
...
int main()
{
    ...
    double lastTitleUpdateTime = glfwGetTime();
    double fps = 0.0;
    ...
    for (int i = 0; i < 4 * N; ++i)   // 线
    {

        add(new Line({ dist(rng), dist(rng) }, { dist(rng), dist(rng) },
            vec4(col(rng), col(rng), col(rng), 1), 1.5f + 3.f * col(rng)));
    }

    for (int i = 0; i < 3 * N; ++i)  // 圆
    {

        add(new Circle({ dist(rng), dist(rng) }, 5 + 80 * col(rng),
            vec4(col(rng), col(rng), col(rng), 1), 2.f));
    }

    for (int i = 0; i < 3 * N; ++i)  // 多段线
    {
        int n = 3 + rng() % 20;
        vector<vec2> pts;
        pts.reserve(n);
        ...
        {
            pts.push_back(p);
            p += vec2(dist(rng) * 0.1f, dist(rng) * 0.1f);
        }
    ...
    while (!glfwWindowShouldClose(win))
    {
        ...
        // 每0.5秒更新一次标题栏
        if (currentTime - lastTitleUpdateTime >= 0.5)
        {
            // 计算FPS
            ...
            // 更新标题栏，显示FPS
            char title[256];
            sprintf(title, "2D CAD - 1M primitives + RTree + GeometryShader (%.1f FPS)", fps);
